...
GROK_MODEL = os.getenv("GROK_TEACHER_MODEL", "grok-4")
...
def _format_trace(trace_data: dict[str, Any]) -> str:
    ...
    for s in steps[:15]:
        ...
        if isinstance(act, dict):
            tool = act.get("tool") or act.get("action") or str(act)[:80]
        else:
            tool = str(act)[:80]
        obs = (s.get("observation") or "")[:200]
        reason = (s.get("reasoning") or s.get("thought") or "")[:120]
...
def _trace_failed(trace_data: dict[str, Any]) -> bool:
    if "success" in trace_data:
        return not bool(trace_data.get("success"))
    if "final_success" in trace_data:
        return not bool(trace_data.get("final_success"))
    return True
...
def generate_training_data(
    ...
    trace_files = sorted(
        [*traces_dir.rglob("*.json"), *traces_dir.rglob("*.jsonl")]
    )[:max_traces]
    ...
    with output_path.open("w", encoding="utf-8") as out_f:
        for trace_file in trace_files:
            ...
            if not _trace_failed(trace_data):
                continue
